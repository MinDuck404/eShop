...
router.get(`/thirdsubCatId`, async (req, res) => {
  let productList = [];
  ...
  if (req.query.page !== undefined && req.query.perPage !== undefined) {
    console.log(req.query.thirdsubCatId )
    const productListArr = await Product.find({ thirdsubCatId: req.query.thirdsubCatId })
      .populate("category")
      .skip((page - 1) * perPage)
      .limit(perPage)
      .exec();
...
router.get(`/fiterByPrice`, async (req, res) => {
  ...
  if (req.query.catId !== "" && req.query.catId !== undefined) {
    ...
    const productListArr = await Product.find().populate("category");
